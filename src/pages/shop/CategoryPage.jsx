...
import { getProductsBySubCategory, getProductsByCategory } from "../../data/productService"
...
const CategoryPage = () => {
  ...
  const subCategoryLabel = subCategory ? formatLabel(subCategory) : categoryLabel
  ...
  const filterProducts = (products, selected) => {
    return products.filter((p) => {
      ...
      let matchPrice = true
      if (selected.priceRange === "Under ₹1,000") matchPrice = numericPrice < 1000
      else if (selected.priceRange === "₹1,000 - ₹3,000") matchPrice = numericPrice >= 1000 && numericPrice <= 3000
      else if (selected.priceRange === "₹3,000 - ₹5,000") matchPrice = numericPrice >= 3000 && numericPrice <= 5000
      else if (selected.priceRange === "Over ₹5,000") matchPrice = numericPrice > 5000
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (subCategoryLabel && subCategoryLabel !== categoryLabel) {
        data = await getProductsBySubCategory(subCategoryLabel)
      } else {
        data = await getProductsByCategory(categoryLabel)
      }
      if (isCurrent) {
        setProducts(data)
        setLoading(false)
    ...
    <div className="bg-white py-6 min-h-screen">
      <Container>
        <div className="flex flex-col gap-2 mt-4 mb-6">
          <h1 className="font-libre font-bold text-[#163422] text-3xl sm:text-4xl lg:text-5xl">
            {subCategoryLabel || categoryLabel}
          </h1>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-6 mt-6 pb-16">
          {visibleProducts.length > 0 ? (
            visibleProducts.map((p) => (
              <ProductCard key={p.id} product={p} fromLabel={subCategoryLabel || categoryLabel} />
            ))
          ) : (
